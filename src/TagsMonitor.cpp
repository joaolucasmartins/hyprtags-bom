...
void TagsMonitor::gotoTag(uint16_t tag) {
  ...
  this->mainTag = tag;
  HyprlandAPI::invokeHyprctlCommand("dispatch",
                                    "workspace " + this->getWorkspaceName(tag));
}
...
void TagsMonitor::flipTag(uint16_t tag) {
  ...
  PHLWORKSPACE otherWorkspace =
      g_pCompositor->getWorkspaceByName(this->getWorkspaceName(tag));
  if (otherWorkspace) {
    auto otherMonitor =
        g_pCompositor->getMonitorFromID(otherWorkspace->m_iMonitorID);
    ...
    if (otherWorkspace->m_iMonitorID != GET_CURRENT_MONITOR()->ID &&
      ...
      HyprlandAPI::invokeHyprctlCommand(
          "dispatch",
          std::format("swapactiveworkspaces {} {}", GET_CURRENT_MONITOR()->ID,
                      otherWorkspace->m_iMonitorID));
    } else {
      HyprlandAPI::invokeHyprctlCommand(
          "dispatch", std::format("focusworkspaceoncurrentmonitor {}",
                                  getWorkspaceName(tag)));
    }
  } else {
    HyprlandAPI::invokeHyprctlCommand(
        "dispatch", std::format("focusworkspaceoncurrentmonitor {}",
...
bool TagsMonitor::activateTag(uint16_t tag) {
  ...
  std::unordered_set<CWindow*> borrowedWindows =
      getWindowsOnWorkspace(this->getWorkspaceName(tag));
  ...
  for (auto w : borrowedWindows) {
    HyprlandAPI::invokeHyprctlCommand(
        "dispatch", std::format("movetoworkspacesilent {},address:0x{:x}",
...
bool TagsMonitor::deactivateTag(uint16_t tag) {
  ...
  for (auto& w : this->borrowedTags[tag]) {
    HyprlandAPI::invokeHyprctlCommand(
        "dispatch", std::format("movetoworkspacesilent {},address:0x{:x}",
                                borrowedWorkspaceName, (uintptr_t)w));
  }
...
void TagsMonitor::altTab() {
  PHLWORKSPACE otherWorkspace = g_pCompositor->getWorkspaceByName(
      this->getWorkspaceName(this->histMainTag));
  if (otherWorkspace) {
    CMonitor* otherMonitor =
        g_pCompositor->getMonitorFromID(otherWorkspace->m_iMonitorID);
    std::string otherMonitorFocusedWorkspace =
        otherMonitor->activeWorkspace->m_szName;
    if (otherWorkspace->m_iMonitorID != GET_CURRENT_MONITOR()->ID &&
        otherMonitorFocusedWorkspace == otherWorkspace->m_szName) {
      HyprlandAPI::invokeHyprctlCommand(
          "dispatch",
...
bool TagsMonitor::isValidTag(uint16_t tag) { return 1 <= tag && tag <= 10; }
